    ...
    const cargar = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('inventario')
          .select('id, producto, cantidad_actual, unidad, ubicacion, origen, minimo, maximo')
          .order('producto', { ascending: true });

        if (error) throw error;
        setInventario((data as InventarioItem[]) ?? []);
      } catch (error) {
        alert(error instanceof Error ? error.message : 'Error cargando inventario general');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <main style={styles.page}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <PageLogo />
        <h1 style={styles.title}>Inventario General (MX + USA)</h1>

        {loading ? (
          <p style={{ color: '#64748B' }}>Cargando...</p>
        ) : inventario.length === 0 ? (
          <div style={styles.emptyBox}>No hay inventario registrado.</div>
        ) : (
